...
_pkg = ModuleType("h3c_tv_child")
_pkg.__path__ = [str(_BASE)]
sys.modules["h3c_tv_child"] = _pkg


def _load(name: str):
    path = _BASE / f"{name}.py"
    spec = importlib.util.spec_from_file_location(f"h3c_tv_child.{name}", path)
    assert spec and spec.loader
    mod = importlib.util.module_from_spec(spec)
    sys.modules[f"h3c_tv_child.{name}"] = mod
    spec.loader.exec_module(mod)
    return mod


_load("const")
ChildPolicyManager = _load("child_policy").ChildPolicyManager
