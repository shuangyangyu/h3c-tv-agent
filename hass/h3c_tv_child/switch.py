"""Switch platform for H3C TV Child."""
...
from .coordinator import ChildPolicyDenied, H3CTVChildCoordinator
...
async def async_setup_entry(
    ...
    """Set up TV internet proxy and child-control switches."""
...
class H3CTVBaseSwitch(CoordinatorEntity[H3CTVChildCoordinator], SwitchEntity):
    """Base class for H3C TV Child switches."""
...
class H3CTVInternetSwitch(H3CTVBaseSwitch):
    """Proxy switch: child-gated control over the MQTT internet switch."""
    ...
    @property
    def available(self) -> bool:
        """Available when the bound MQTT switch is present."""
        status = (
            self.coordinator.data.get("statuses", {}).get(self.tv_key, {})
            if self.coordinator.data
            else {}
        )
        return super().available and bool(status.get("switch_available"))
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        """Expose automatic-control and binding info."""
        if not self.coordinator.data:
            return {}
        ...
        return {
            ...
            "mqtt_switch_entity_id": status.get("mqtt_switch_entity_id"),
